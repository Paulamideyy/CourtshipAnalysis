...
def find_continuous_ones(data):
    ...
    Args:
        data: A list or Series representing the data containing only 1s and 0s.

    Returns:
        A list of tuples, where each tuple represents a continuous series of 1s with (start_index, end_index).
    """
    results = []
    start = None
    for i, v in enumerate(data):
        if v == 1 and start is None:
            ...
            results.append((start, i - 1))  # Adjusted to end at i - 1
            start = None
...
# Function to get user input for column name
def get_column_name():
    while True:
...
def process_file(file_path, column_name):
    try:
        # Load the TSV file
        data = pd.read_csv(file_path, sep='\t', header=None, error_bad_lines=False, low_memory=False)
        ...
        if column_name in headers:
            ...
            if results:
                ...
                # Extract the 5th component of the path
...
# Iterate through each subfolder and process track.tsv files
for root, dirs, files in os.walk(main_folder):
    for name in files:
        if name == "track.tsv" and ("DetectedArena_1" in root or "CustomArena_" in root):  # Only process track.tsv files inside DetectedArena_1 or CustomArena_ folders
